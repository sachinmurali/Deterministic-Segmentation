...
def maxMatch(sentence, fList):
	...
	for i in range(len(sentence), 0, -1):
		...
		if i == 1:
			...
			finalList.extend(maxMatch(remainder, fList))
			break

		if fList.get(firstword) != None:
			finalList.append(firstword)
			finalList.extend(maxMatch(remainder, fList))
			break			
...
def splitTokens(tokenList):
	with open("oldmaxmatch.txt", "a") as hFile:							
...
def minEditDist(source, target):
   ...
   D[0][0] = 0                                                             # Assign the element in the matrix as 0
